...
function preload(){
    tab_img = loadImage("solid-wood-dining-table-1.jpg");
}
...
function draw(){
    ...
    if(Status != ""){
        for(i = 0; i < objects.length; i++){
            ...
            text(objects[i].label + " " + percent + "%",objects[i].x - 200, objects[i].y - 190);
            noFill();
            console.log(objects[i].x +","+ objects[i].y)
            console.log(objects[i].width +","+ objects[i].height)
            stroke("#fc0303");
            rect(objects[i].x -200, objects[i].y - 190, objects[i].width, objects[i].height);
        }
    }
}